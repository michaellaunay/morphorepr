...
> **Milestone note (docs-as-code, v6.10.0 — step 1 completed).** Since v6.10.0, the **Python repository is the source of truth for code** (option B, recorded in `morphorepr_note_etape_orientation_v6.10.0.md`). The repository has been **materialized** from this document, and the suite now runs under native `pytest`: **121 passed / 1 skipped** (the historical “69” count corresponded to the subset replayed by the previous extraction harness). The code blocks below are a **v6.9.0 snapshot kept for methodological reference**; in case of divergence, **the repository is authoritative**. Two known divergences, at the test level only (production code unchanged), were fixed in the repository and are not backported here: (1) `tests/test_model_providers.py` — the `_PRIMARY` fixture now provides `inference_env_hash` (required by `_REQUIRED_FULL_RUN_ARTIFACTS`); (2) `tests/test_model_run_propagation.py` — `ensure_legacy_model_run("r1")` is called before explicitly passing `model_run_id="r1::legacy"` (FK constraint, `PRAGMA foreign_keys=ON`). Some “to do” instructions in this document (for example, the README update) have since been completed. The next milestone (end-to-end Phase 4 orchestrator, corrected order `p4_steer → p4_predict → p4_predict_baselines → p4_score → p4_controls → p4_dev_summary`) is specified in the milestone note.
