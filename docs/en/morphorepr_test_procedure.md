...
> **Milestone note (docs-as-code, v6.10.0 — step 1 done).** Since v6.10.0, the **Python repository is the source of truth for the code** (option B, decided in `en_morphorepr_milestone_note_orientation_v6.10.0.md`). The repository has been **materialized** from this document, and the suite now runs under native `pytest`: **121 passed / 1 skipped** (the historical “69” count was the subset replayed by the old extraction harness). The code blocks below are a **v6.9.0 snapshot kept as the methodological reference**; in case of divergence, **the repository prevails**. Two known divergences, at test level only (production code unchanged), fixed in the repository and not back-ported here: (1) `tests/test_model_providers.py` — the `_PRIMARY` fixture now provides `inference_env_hash` (required by `_REQUIRED_FULL_RUN_ARTIFACTS`); (2) `tests/test_model_run_propagation.py` — `ensure_legacy_model_run("r1")` is called before explicitly passing `model_run_id="r1::legacy"` (FK constraint, `PRAGMA foreign_keys=ON`). Some “to do” instructions in this document (e.g. the README update) have since been carried out. The next milestone (Phase 4 end-to-end orchestrator, corrected order `p4_steer → p4_predict → p4_predict_baselines → p4_score → p4_controls → p4_dev_summary`) is specified in the milestone note.
